# Build HTML files with contacts
param([string]$city="phuket", [string]$cityName="Phuket")

Write-Host "Starting generation..."

# Data tables using direct Unicode strings
$catDefs = @()
$catDefs += @{id="beauty";    name=@"
Красота и уход
"@; n=62}
$catDefs += @{id="fitness";   name=@"
Фитнес и спорт
"@; n=52}
$catDefs += @{id="kids";      name=@"
Дети и развитие
"@; n=90}
$catDefs += @{id="psychology";name=@"
Психология и здоровье
"@; n=38}
$catDefs += @{id="medicine";  name=@"
Медицина
"@; n=28}
$catDefs += @{id="photo";     name=@"
Фото и видео
"@; n=33}
$catDefs += @{id="rental";    name=@"
Аренда и транспорт
"@; n=38}
$catDefs += @{id="tourism";   name=@"
Туризм и экскурсии
"@; n=38}
$catDefs += @{id="food";      name=@"
Еда и продукты
"@; n=28}
$catDefs += @{id="digital";   name=@"
Цифровые услуги
"@; n=28}
$catDefs += @{id="realty";    name=@"
Недвижимость
"@; n=18}
$catDefs += @{id="other";     name=@"
Прочее
"@; n=28}

# Trim newlines from names
$catDefs = $catDefs | % { $_.name = $_.name.Trim(); $_ }
...
$svc.food = "Домашняя выпечка|Торты на заказ|Русские продукты|Морепродукты, доставка|Кофе, чай|Мясо, птица|Фрукты, овощи|Кондитерская ручной работы|Здоровое питание|Молочная продукция|Суши, роллы|BBQ, шашлык|Вегетарианская кухня|Хлеб, выпечка|Орехи, снеки|Кейтеринг|Мед, прополис|Полуфабрикаты|Сыры домашние|Доставка воды" -split '\|'
$svc.digital = "SMM-менеджер|Разработка сайтов|Таргетолог|Графический дизайн|Копирайтер|SEO-продвижение|Контент-план|Создание Telegram-ботов|Видеопродакшн|Мобильные приложения|Брендинг|Веб-аналитика|CRM-внедрение|Переводчик|Техподдержка|Моушн-дизайн|AI-услуги|Лендинги|Настройка VPN|Хостинг, домены" -split '\|'
$svc.realty = "Аренда квартир долгосрочно|Аренда домов, вилл|Продажа недвижимости|Кондо, новостройки|Управление арендой|Земельные участки|Дизайн интерьера|Ремонт под ключ|Посуточная аренда|Юрист по недвижимости|Инвестиции|Коммерческая аренда|Виллы с бассейном|Гестхаусы|Аренда комнаты" -split '\|'
$svc.other = "Юридические консультации|Ремонт ПК, ноутбуков|Цветы, букеты|Химчистка, стирка|Клининг, уборка|Груминг собак|Передержка животных|Швейное ателье|Мебель на заказ|Страхование|Изготовление ключей|Обмен валюты|Украшения ручной работы|Организация праздников|Ремонт телефонов|Международная доставка|Нотариус|Ветеринар|Автосервис|Изготовление печатей|Сувениры, подарки" -split '\|'
...
'@)
    ...
    $o.Add("  <div class=""meta"">AION &middot; $total контактов &middot; 12 категорий</div>")
    $o.Add('</div>')
    ...
    [System.IO.File]::WriteAllText($filePath, ($o -join "`n"), [System.Text.Encoding]::UTF8)
